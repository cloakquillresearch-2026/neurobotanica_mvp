...
async def analyze(request: AnalysisRequest, consent_header: str = Header(None, alias="X-Consent-ID"), db=None):
    ...
    try:
        ...
        timings["synergy"] = (time.time() - last) * 1000
        last = time.time()
        ...
        timings["plant_profile"] = (time.time() - last) * 1000
        last = time.time()
        ...
        timings["polysaccharide_effects"] = (time.time() - last) * 1000
        # Combine results
        result = {
